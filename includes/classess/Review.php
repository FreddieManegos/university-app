...
class Review {
	...
	public function submitReview($body, $course_to){
		...
		if($check_empty != "") {
			...
			$query = pg_query($this->db, "INSERT INTO reviews(body, added_by, course_to, date_added, user_closed, deleted) VALUES ('$body','$added_by','$course_to','$date_added','no','no')");
		}

	}

	public function loadPostsReview() {
		$str = ""; //String to return
		$course_to = $this->course_obj->getCourseCode();
		$data = pg_query($this->db, "SELECT * FROM reviews WHERE course_to = '$course_to' AND deleted= 'no' ORDER BY id DESC");
		while($row = pg_fetch_array($data)) {
			...
			$comments_check_num  = pg_num_rows($comments_check);
					...
					$str .= "<div class='status_post_discussionRev' onClick='javascript:toggle$id()' >
					<div class='post_profile_pic'>
					<img src='assets/Images/review_name.png' width='50'>
					</div>

					<div class='posted_by' style='color:#ACACAC;'>
					<a href='$added_by'> Anonymous &nbsp;&nbsp; &nbsp;&nbsp;</a> ($course_to) &nbsp;&nbsp;&nbsp;&nbsp;$time_message
					</div>
					<div id='post_body'>
					$body";
					}
