...
<?php 
	...
	if(isset($_GET['post_id'])) {
 	...
 	$get_likes = pg_query($db, "SELECT likes, added_by FROM discussions WHERE id = '$post_id'");
 	...
 	if(isset($_POST['like_button'])){
 		$total_likes++;
 		$query = pg_query($db, "UPDATE discussions SET likes='$total_likes' WHERE id='$post_id'");
 		$insert_user = pg_query($db, "INSERT INTO likes(username,post_id) VALUES('$userLoggedIn','$post_id')");
 	}
 	//For the UNlike button
	if(isset($_POST['unlike_button'])){
 		$total_likes--;
 		$query = pg_query($db, "UPDATE discussions SET likes='$total_likes' WHERE id='$post_id'");
 		$insert_user = pg_query($db, "DELETE FROM likes WHERE username='$userLoggedIn' AND post_id='$post_id'");
 	}
 	$check_query = pg_query($db,"SELECT * FROM likes WHERE username = '$userLoggedIn' AND post_id='$post_id'");
